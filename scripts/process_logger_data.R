...
b1 <- data.table::fread("~/Desktop/20220404_hawaii_temp_button1.csv") %>%
  ...
  dplyr::filter(datetime > lubridate::ymd_hms("2022-4-1 21:00:00")) %>%
  dplyr::select(datetime, timezone, temp_c = Value, logger, high_alarm) %>%
  dplyr::arrange(datetime)

b2 <- data.table::fread("~/Desktop/20220404_hawaii_temp_button2.csv") %>%
  tidyr::separate(`Date/Time`, into = c("date", "time", "unit"), sep = " ") %>%
  dplyr::mutate(time2 = lubridate::hms(time),
                time3 = case_when(unit == "PM" ~ time2+lubridate::hours(12),
                                  unit != "PM" ~ time2),
                ...
                logger = "Big Island",
                high_alarm = ifelse(Value >= 27, T, F)) %>%
  dplyr::filter(datetime > lubridate::ymd_hms("2022-3-31 17:00:00")) %>%
  dplyr::select(datetime, timezone, temp_c = Value, logger, high_alarm) %>%
  dplyr::arrange(datetime)
...
bound_df <- dplyr::bind_rows(b1, b2)
